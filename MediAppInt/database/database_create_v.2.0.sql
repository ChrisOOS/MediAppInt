...
CREATE TABLE IF NOT EXISTS `mirthdb`.`visit` (
  ...
  CONSTRAINT `fk_visit_patient1`
    ...
    REFERENCES `mirthdb`.`patient` (`pid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
AUTO_INCREMENT = 17
...
CREATE TABLE IF NOT EXISTS `mirthdb`.`Lab_Orders` (
  ...
  CONSTRAINT `fk_Lab_Orders_visit1`
    FOREIGN KEY (`visit_vid` , `visit_patient_pid`)
    REFERENCES `mirthdb`.`visit` (`vid` , `patient_pid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Lab_Orders_Provider1`
    FOREIGN KEY (`Provider_providerID`)
    REFERENCES `mirthdb`.`Provider` (`providerID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mirthdb`.`Lab_Results` (
  ...
  `placerNum` VARCHAR(45) NULL,
  `fillerOrderNum` VARCHAR(45) NULL,
  `UniversalServiceIdentifier` VARCHAR(45) NULL,
  `dateTransaction` VARCHAR(45) NULL,
  `observationValue` VARCHAR(45) NULL,
  `observationIdentifier` VARCHAR(45) NULL,
  `observationResultStatus` VARCHAR(45) NULL,
  ...
  CONSTRAINT `fk_Lab_Results_Lab_Orders1`
    FOREIGN KEY (`Lab_Orders_labOrderNum` , `Lab_Orders_visit_vid` , `Lab_Orders_visit_patient_pid`)
    REFERENCES `mirthdb`.`Lab_Orders` (`labOrderNum` , `visit_vid` , `visit_patient_pid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
